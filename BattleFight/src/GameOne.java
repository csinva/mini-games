   ...
   public class GameOne extends JPanel{////////////////add grenades
		...
		boolean u,r,d,l,move,space,grenade,added,reached=false;
		AccountListener AL;
		JButton scores;
		...
		int startLevel=0;
		int level=0;
		int grenadeCount,gM=0;
		int kills,numRemaining,numAlive,slower,slower2,pslower,pauser,score,end=0;
		double hits, shots=0;
      ...
      public GameOne(int a){
		startLevel=level=a;
			...
			for(int x=0;x<numRBullets;x++){
				rBullets[x]=new Bullet(0,0,6,5,Color.black,1);
				rBullets[x].setOn(2);
         ...
         setFocusable(true);
			...
			p=new Rectangle(40,40,20,2,Color.green,50);
			p.draw(g);
		...
		private class TimerListener implements ActionListener{
         public void actionPerformed(ActionEvent e){
			   ...
			   slower++;
				...
				slower2++;
				if(slower2==65){
					if(p.getHealth()<50&&p.getHealth()>0)
				...
				for(int x=0;x<numRemaining;x++){
						if(enemies[x].getAlive()){
							enemies[x].draw(g);
								if(collided(enemies[x])&&!enemies[x].isAShooter()){
									if(enemies[x].getAttacking()>0&&enemies[x].getAttacking()<35){
         ...
         public void keyPressed(KeyEvent e){
			...
			if(kListenerOn){ 
				switch(e.getKeyCode()){
					...
					// case KeyEvent.VK_T:
// 						p.setGun(3);
// 						p.setBulletColor(Color.yellow);
// 						p.setShotFrequency(12);
// 						p.setShot(0);break;
					case KeyEvent.VK_G:
						shootGrenade(p);break;
					// case KeyEvent.VK_U:///////////implements Uzi
// 						p.setGun(2);
// 						p.setBulletColor(Color.gray);
// 						for(int x=0;x<numUBullets;x++)
// 							uBullets[x].setOn(0);
// 						p.setShotFrequency(6);
// 						p.setShot(0);break;
// 					case KeyEvent.VK_Y://////////implements switching back to pistol
// 						p.setGun(1);
// 						p.setBulletColor(Color.green);
// 						p.setShotFrequency(25);
// 						p.setShot(0);break;
				}
				...
				if(boxes[x].getOn()==1){
					...
					if(level<10){
						...
						if(p.getHealth()<35){
							...
							for(int a=0;a<numRBullets;a++){
								if(rBullets[a].getOn()!=1)
									rBullets[a].setOn(0);
							}
							addLabel("+Rockets",boxes[x].getX(),boxes[x].getY());
						}
      ...
      public void moveRect(Rectangle a){	///////////////////////////////////////MOVING AND SHOOTING
		...
		public void moveGrenade(Grenade a){
				grenadeCount++;/////////////////////use this
				int oldX=a.getX();
				int oldY=a.getY();
				int l=a.getLifeTime();
				switch(a.getDIR()){//moves them as well
					case 1:
					case 5:
						if(l<15){
							a.setSpeed(5);
...
// 				rBullets[x].setRocket(true);
      ...
      public void drawBackground(int lev){///////////////////////////////////////////////////BACKGROUND AND LEVELS
			ImageIcon i=new ImageIcon("pics/space.jpg");
			switch(lev){
			case 0:
			case 1:
...
////////////////THIS CODE WAS BORROWED//////////////http://www.dreamincode.net/forums/topic/162705-dispose-a-frame-from-an-inner-class/////
         		...
         		frame.setSize(920,960);  
        			...
        			frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
       		   frame.setContentPane(new Account(1,myX));
        			frame.setVisible(true);
			}
		}
      public void paintComponent(Graphics a){
		/*								//////////////save this for team deathmatch
		int scale=2;
		int bx=0;
		int by=0;
		...
		*/
		...
		s2.update(p.getGun(),uziAmmo,grenadeAmmo, rAmmo,level);
		}
